	...
	if request.method == 'POST':
		...
		for p in predictions1:
			if(p['probability']>0.35):
				ind=predictions1.index(p)
				pred_prob=str(round(p['probability']*100,2))
				pred_out_list.append((p['tagName'],pred_prob))
				if p['tagName']=='Balaji':
					classes[ind]=append(1)
					scores[ind]=p['probability']
				elif p['tagName']=='CocaCola':
					classes[ind]=2
					scores[ind]=p['probability']
				elif p['tagName']=='Kurkure':
					classes[ind]=3
					scores[ind]=p['probability']
				elif p['tagName']=='Lays':
					classes[ind]=4
					scores[ind]=p['probability']
				elif p['tagName']=='Sprite':
					classes[ind]=5
					scores[ind]=p['probability']
		print(pred_out_list)
		print(classes)
		print(scores)
		classes1.append(classes)
...
def showmap():
	return render_template('map_pwp.html')'''
